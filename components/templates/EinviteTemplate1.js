...
export default function EinviteTemplate1({ invitation = {} }) {
  ...
  // Page 6 height adapts to event count: compact for ≤2 events, taller for 3+
  const page6Height = events.length <= 2 ? 760 : 960;
  const page6Top = hasPhotos ? 3804 : 3039;
  const page7Top = page6Top + page6Height;

  // Canvas total height adjusts with dynamic page 6
  const totalHeight = page7Top + 760; // page7 is always 760
  ...
  // When no gallery → canvas is shorter → same pixel positions map to higher %
  const templeStartPct = hasPhotos ? '7.42%' : '8.62%';
  const templeEndPct = hasPhotos ? '11.77%' : '13.7%';
  ...
  const carpetClipPath = useTransform(
    scrollYProgress,
    [1064 / totalHeight, 1703 / totalHeight],
  ...
  const carpetTextOpacity = useTransform(
    scrollYProgress,
    [1200 / totalHeight, 1703 / totalHeight],
    [0, 1]
  );
  ...
  return (
    <div className={styles.wrapper}>
      <div
        ...
        ref={canvasRef}
        style={{ aspectRatio: `1440 / ${totalHeight}` }}
      >
        ...
        <div
          ...
          style={{ top: `calc(${page6Top} / 1440 * 100cqw)`, height: `calc(${page6Height} / 1440 * 100cqw)` }}
        >
        ...
        <div
          ...
          style={{ top: `calc(${page7Top} / 1440 * 100cqw)` }}
        >
          ...
          >
            ...
            <div className={styles.thanksText}>
              ...
              With love,<br />
              <span style={{ fontWeight: 700, fontSize: '2.8cqw' }}>{groomName} & {brideName}</span>
            </div>
